  ...
  <div class="outer">
    <TableView :initArrData="initArr" :totalCount="tableTotalNum" :loading="tableLoading" @searchTable="getTableData">
      ...
      <div slot="tableAction" slot-scope="slotPropsData">
        <a v-if="slotPropsData.currRowdata.isInware === '1'" href="javascript:;" data-type="已入库" class="successBtnColor">已入库</a>
        <a v-if="slotPropsData.currRowdata.isInware === '2'" href="javascript:;" data-type="已转出" class="primaryBtnColor">已转出</a>
        <a v-if="slotPropsData.currRowdata.isInware === '0' || slotPropsData.currRowdata.borrowState==='1'" href="javascript:;" data-type="借出" class="warnBtnColor" @click="operateFun(slotPropsData.currRowdata, 'checkout')">借出</a>
        <a v-if="slotPropsData.currRowdata.isInware === '0' && slotPropsData.currRowdata.borrowState==='1'" href="javascript:;" data-type="归还" class="infoBtnColor" @click="operateFun(slotPropsData.currRowdata, 'return')">归还</a>
        <a v-if="slotPropsData.currRowdata.isInware === '0' && slotPropsData.currRowdata.borrowState==='1'" href="javascript:;" data-type="打印预览" @click="operateFun(slotPropsData.currRowdata, 'print')">打印预览</a>
      </div>
    </TableView>

    <!-- 查阅记录、档案借出、归还modal -->
    <div class="addModal">
      <a-modal centered :visible="visible" :confirmLoading="confirmLoading" :width="tempOperateVal !== 'return' ? '80%' : '40%'" @cancel="handleCancel" :maskClosable="false" style="height:85%;overflow: hidden;" :footer="tempOperateVal==='checkRecord' ? null: void 0">
    ...
    <!-- 打印预览--modal -->
    <div class="addModal">
      <a-modal centered :visible="printVisible" :confirmLoading="printConfirmLoading" :width="modalWidth" @cancel="printCancel" style="height:85%;overflow: hidden;" :maskClosable="false">
        <div slot="title" class="roomModalTitleSlot">
          <p>交接清单预览</p>
          <span>{{currentRowData && currentRowData.a0101}}</span>
        </div>
        ...
        <div class="modalInnerContainer">
          <TemplateOfPrint :fileTopNum="fileNum" IOU='true' :firstTitle="firstTitle" :secondTitle="secondTitle" ref="print">
            <div slot="printContent" class="printContent">
              <p class="indent">
                今借到<span class="redSpan">{{currRow && currRow.a0101}}</span>同志（档案管理编号：<span class="noFontColorSpan printConNumFontFamily">{{currRow && currRow.a0100a}}</span>)档案材料：<span class="redSpan">{{currRow && currRow.expectReturnDate}}</span>供<input type="text" v-model="printData.useName" style="width:150px;" />使用。
              </p>
              <div class="bottom">
                <div class="bottomRight">
                  <p>具借人:{{currRow && currRow.borrower}}</p>
                  <p>证件号:<span class="printConNumFontFamily">{{currRow && currRow.borrowerIdNum}}</span></p>
                  <p>联系电话:<span class="printConNumFontFamily">{{currRow && currRow.borrowerTelNum}}</span></p>
                  <p>日期：<span v-html="nowData"></span></p>
                </div>
              </div>
            </div>
          </TemplateOfPrint>
...
export default {
  ...
  methods: {
    getTableData(condition, pageNum, limitNum) {
      ...
      this.$http
        .fetchPost('materialBorrow@getCatalogBorrowList.action', {
          page: pageNum,
          limit: limitNum,
    ...
    getAgentFun(currRowData) {
      ...
      if (currRowData) {
        const _this = this
        _this.$http
          .fetchPost('materialBorrow@catalogBorrowRecord.action', {
    ...
    handleOK() {
      ...
      if (resultForm['isRight']) {
        ...
        if (_this.tempOperateVal === 'checkout') {
          ...
          _this.$http
            .fetchPost('materialBorrow@saveCatalogBorrow.action', currPostObj)
            .then(res => {
              if (Number(res.code) === 0) {
    ...
    printOperateFun(currRowData) {
      ...
      const _this = this
      _this.$http
        .fetchPost('materialBorrow@materialPrint.action', {
          a0101: currRowData.a0101,
